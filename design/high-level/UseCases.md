...
一旦wasm被宿主调用，下一个设计选择就是在运行中如何暴露宿主的本地功能和资源暴露给wasm代码。
...
3. 宿主通过语言中立的接口将非阻塞和/或流式I/O暴露给组件。
   宿主通过语言中立的接口将非阻塞和/或流式I/O暴露给组件，这些接口可以绑定到不同组件的
   源语言的并发功能（如承诺、期货、异步/等待和珊瑚）。
   async/await和coroutines）。
4. 宿主将配置（例如，来自配置文件和秘密的值）传递给组件。
   宿主通过导入类型化的高级值和句柄，将配置（如配置文件和秘密的值）传递给组件。
5. 一个组件声明一个特定的导入是 "可选的"，允许该
   组件可以在宿主上执行
6. 开发者在生产中用本地宿主导入实例化一个组件，并在本地开发和测试中使用模拟或仿真的导入。
