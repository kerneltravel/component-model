...
当宿主嵌入wasm时，另一个设计选择是何时创建新的实例。
何时将事件传送给现有的实例，何时销毁现有的实例，以及如果有多个运行的实例，它们如何相互作用，如果有需要的话。这个场景包括：

1. 一台宿主创建了许多短暂的、并发的组件实例，其中每一个实例都与一个特定的宿主-数据库绑定。
   都与一个特定的宿主域实体的生命周期相关联（例如，一个请求-响应对、连接）。
